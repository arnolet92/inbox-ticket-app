...
const ADMINS_KEY = "inbox_ticket_admins";
...
export default function AdminUsers() {
  ...
  const isSelf = (admin: AdminAccount) =>
    user?.phone?.replace(/\s/g, "") === admin.phone.replace(/\s/g, "");

  const isLastSuperAdmin = (admin: AdminAccount) =>
    admin.role === "super_admin" && admins.filter((a) => a.role === "super_admin").length === 1;

  const canDelete = (admin: AdminAccount) => !isSelf(admin) && !isLastSuperAdmin(admin);
                ...
                <TableRow key={admin.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {isSelf(admin) && (
                          <div className="text-xs text-accent font-medium">Vous</div>
                        )}
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    {canDelete(admin) ? (
                      ...
                      <span className="text-xs text-muted-foreground/40 px-3">
                        {isLastSuperAdmin(admin) ? "Protégé" : "—"}
                      </span>
                    )}
                  </TableCell>
                </TableRow>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 pr-11 rounded-xl border border-border bg-muted/40 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                  />
                  <button
                    type="button"
