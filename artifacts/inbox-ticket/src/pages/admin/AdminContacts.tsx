...
import {
  ...
  UserCheck, UserCircle, Users, Download, Filter,
} from "lucide-react";

type FilterType = "all" | "with_account" | "buyer_only";
...
function getAccounts(): { name: string; address: string; phone: string }[] {
...
export default function AdminContacts() {
    ...
    (orders ?? []).forEach((order) => {
      ...
      if (map.has(key)) {
        ...
        if (order.status === "confirmed") {
          ...
          c.totalTickets += order.quantity ?? 0;
        }
        if (!c.lastOrderDate || new Date(order.createdAt) > new Date(c.lastOrderDate)) {
        ...
        map.set(key, {
          phone: raw,
          name: order.customerName ?? raw,
          email: order.customerEmail ?? "",
          address: "",
          hasAccount: false,
          orderCount: 1,
          confirmedOrders: order.status === "confirmed" ? 1 : 0,
          totalSpent: order.status === "confirmed" ? (order.totalAmount ?? 0) : 0,
          totalTickets: order.status === "confirmed" ? (order.quantity ?? 0) : 0,
  ...
  const stats = useMemo(() => ({
    ...
    withAccount: contacts.filter((c) => c.hasAccount).length,
    buyerOnly: contacts.filter((c) => !c.hasAccount && c.orderCount > 0).length,
    totalRevenue: contacts.reduce((s, c) => s + c.totalSpent, 0),
  }), [contacts]);

  const filtered = useMemo(() => {
    let list = contacts;
    if (filter === "with_account") list = list.filter((c) => c.hasAccount);
    if (filter === "buyer_only") list = list.filter((c) => !c.hasAccount);
  ...
  function exportCSV() {
    const rows = [
      ["Nom", "Téléphone", "Email", "Adresse", "Compte", "Commandes", "Total dépensé (Ar)"],
      ...filtered.map((c) => [
        c.name, c.phone, c.email, c.address,
        c.hasAccount ? "Oui" : "Non",
    ...
    a.href = url; a.download = "contacts_inbox_ticket.csv"; a.click();
    URL.revokeObjectURL(url);
  }

  const FILTERS: { key: FilterType; label: string }[] = [
    { key: "all", label: "Tous" },
    { key: "with_account", label: "Avec compte" },
    { key: "buyer_only", label: "Acheteurs sans compte" },
  ];
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "Avec compte",       value: stats.withAccount,  icon: UserCheck,   color: "text-accent",      bg: "bg-primary/10" },
          { label: "Acheteurs",         value: stats.buyerOnly,    icon: ShoppingBag, color: "text-blue-400",    bg: "bg-blue-500/10" },
                ...
                <TableRow key={contact.phone}>
                  ...
                  <TableCell>
                    <span className="font-bold text-accent">
                      {contact.totalSpent > 0 ? formatMGA(contact.totalSpent) : "—"}
                    </span>
                  </TableCell>
                  <TableCell className="text-sm text-muted-foreground">
                    {contact.lastOrderDate
                      ? format(new Date(contact.lastOrderDate), "dd MMM yy", { locale: fr })
                      : "—"}
                  </TableCell>
                </TableRow>
              ))}
