...
function Field({
  label, icon, type = "text", value, onChange, placeholder, hint, error, required, rightEl,
}: {
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div>
          <h2 className="text-4xl font-bold font-display text-white leading-tight mb-4">
            Votre accès<br /><span className="text-accent">aux meilleurs</span><br />événements
          </h2>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="flex items-center gap-3 text-sm text-white/70">
              <div className="w-8 h-8 rounded-lg bg-accent/20 flex items-center justify-center text-accent shrink-0">{f.icon}</div>
              {f.text}
            </div>
          ))}
        </div>
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button type="submit" variant="accent" size="lg" className="w-full gap-2 mt-2" disabled={loading}>
          {loading ? <><span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" /> Connexion...</> : <>Se connecter <ArrowRight className="w-4 h-4" /></>}
        </Button>
      </form>
      ...
      <button onClick={onSwitchToRegister} className="w-full py-3 rounded-xl border border-accent/30 text-accent hover:bg-accent/10 font-semibold text-sm transition-all hover:border-accent/60">
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <button onClick={onSwitchToLogin} className="w-9 h-9 rounded-xl border border-border hover:border-accent/50 flex items-center justify-center transition-all hover:bg-accent/10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <Field label="Mot de passe" icon={<Lock className="w-4 h-4" />} type={showPwd ? "text" : "password"}
            ...
            rightEl={<button type="button" onClick={() => setShowPwd(!showPwd)} className="text-muted-foreground hover:text-foreground transition-colors">{showPwd ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>}
          />
          {password && (
            <div className="mt-2 space-y-1">
              <div className="flex gap-1">
                {[1,2,3,4].map(i => <div key={i} className="flex-1 h-1 rounded-full transition-all duration-300" style={{ background: i <= strength ? STRENGTH_COLORS[strength] : "hsl(var(--muted))" }} />)}
              </div>
              <p className="text-xs" style={{ color: STRENGTH_COLORS[strength] }}>{STRENGTH_LABELS[strength]}</p>
            </div>
          )}
        </div>
        <Field label="Confirmer le mot de passe" icon={<Lock className="w-4 h-4" />} type={showConfirm ? "text" : "password"}
          value={confirm} onChange={setConfirm} placeholder="Retapez votre mot de passe" error={errors.confirm} required
          rightEl={<button type="button" onClick={() => setShowConfirm(!showConfirm)} className="text-muted-foreground hover:text-foreground transition-colors">{showConfirm ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>}
        />
        {confirm && !errors.confirm && password === confirm && (
          <div className="flex items-center gap-2 text-xs text-emerald-400"><CheckCircle2 className="w-3.5 h-3.5" /> Les mots de passe correspondent</div>
        )}
        <Button type="submit" variant="accent" size="lg" className="w-full gap-2 mt-2" disabled={loading}>
          {loading ? <><span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" /> Création...</> : <>Créer mon compte <ArrowRight className="w-4 h-4" /></>}
        </Button>
      </form>
      <p className="text-center text-xs text-muted-foreground">
        Déjà un compte ?{" "}
        <button onClick={onSwitchToLogin} className="text-accent hover:underline font-semibold">Se connecter</button>
      </p>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex flex-col min-h-screen bg-background">
        <div className="flex items-center justify-between px-8 py-6 lg:hidden"><Logo /></div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 sm:px-12 py-8">
          <div className="w-full max-w-md">
            <AnimatePresence mode="wait">
              {mode === "login" ? (
                <motion.div key="login" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.25, ease: "easeInOut" }}>
                  <LoginForm onSwitchToRegister={() => setMode("register")} redirectTo={redirectTo} />
                </motion.div>
              ) : (
                <motion.div key="register" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.25, ease: "easeInOut" }}>
        ...
        <div className="px-8 py-5 border-t border-border/30 flex items-center justify-between flex-wrap gap-2">
          <p className="text-xs text-muted-foreground">© {new Date().getFullYear()} Inbox Ticket</p>
