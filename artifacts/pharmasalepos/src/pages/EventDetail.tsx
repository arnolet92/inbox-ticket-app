...
import { useGetEvent } from "@/data/static";
...
export default function EventDetail() {
  ...
  const totalAmount = cartItems.reduce((sum, item) => sum + item.ticket.price * item.qty, 0);
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={i} className="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center text-center">
                  <div className="h-10 w-10 rounded-full bg-primary/20 text-accent flex items-center justify-center mb-3">{item.icon}</div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-28">
              ...
              {!event.ticketTypes?.length ? (
                <Card className="p-6 text-center text-muted-foreground border-dashed">Aucun billet configuré.</Card>
              ) : (
                <div className="space-y-4">
                  {event.ticketTypes.map(ticket => {
                    ...
                    return (
                      <motion.div key={ticket.id} layout animate={{ scale: hasQty ? 1.01 : 1 }} transition={{ duration: 0.2 }}>
                        <Card className={`p-5 transition-all duration-200 ${isSoldOut ? "opacity-50 grayscale" : ""} ${hasQty ? "ring-2 ring-accent border-accent bg-accent/5 shadow-lg shadow-accent/10" : "hover:border-accent/30"}`}>
                          <div className="flex justify-between items-start mb-1">
                            <div>
                              <h4 className="font-bold text-lg leading-tight">{ticket.name}</h4>
                              {ticket.name === "VIP" && <span className="text-xs text-yellow-400 font-semibold">★ Premium</span>}
                            </div>
                            <div className="text-right">
                              <div className="font-display font-bold text-accent text-lg whitespace-nowrap">{formatMGA(ticket.price)}</div>
                              <div className="text-xs text-muted-foreground">/ personne</div>
                            </div>
                          </div>

                          {ticket.description && <p className="text-sm text-muted-foreground mt-1 mb-3">{ticket.description}</p>}
                          ...
                          <div className="mb-4">
                            ...
                            <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                              <div className={`h-full rounded-full transition-all ${isSoldOut ? "bg-destructive" : "bg-accent"}`}
                          ...
                          {!isSoldOut && (
                            <div className="flex items-center justify-between bg-background/60 rounded-xl p-1 border border-border/50">
                              <button onClick={() => updateQty(ticket.id, -1, available)} disabled={qty === 0}
                              ...
                              <div className="flex-1 text-center">
                                {qty === 0 ? (
                                  ...
                                  <motion.span key={qty} initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                                    className="font-bold text-lg text-accent block">{qty}</motion.span>
                                )}
                              </div>
                              <button onClick={() => updateQty(ticket.id, +1, available)} disabled={qty >= available}
                            ...
                            {hasQty && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                <div className="flex justify-between items-center mt-3 pt-3 border-t border-accent/20">
                                  ...
                                  <span className="font-bold text-accent">{formatMGA(ticket.price * qty)}</span>
                                </div>
                              </motion.div>
                            )}
                          </AnimatePresence>
      ...
      <AnimatePresence>
        {totalQty > 0 && (
          <motion.div initial={{ y: 120, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 120, opacity: 0 }}
            transition={{ type: "spring", damping: 20, stiffness: 300 }} className="fixed bottom-0 left-0 right-0 z-50">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 flex-1">
                  <div className="relative">
                    ...
                    <span className="absolute -top-2 -right-2 bg-accent text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{totalQty}</span>
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground">{totalQty} billet{totalQty > 1 ? "s" : ""} sélectionné{totalQty > 1 ? "s" : ""}</div>
                    <div className="font-display font-bold text-xl text-white">{formatMGA(totalAmount)}</div>
                  </div>
                </div>
                <div className="hidden md:flex flex-wrap gap-2">
                  {cartItems.map(item => (
                    <div key={item.ticket.id} className="flex items-center gap-1.5 bg-accent/10 border border-accent/20 rounded-full px-3 py-1 text-sm">
                      ...
                      <button onClick={() => setQuantities(prev => { const { [item.ticket.id]: _, ...rest } = prev; return rest; })}
                ...
                <Button variant="accent" size="lg" className="shrink-0 text-base font-bold px-8 shadow-lg shadow-accent/30 group" onClick={handleCheckout}>
                  Réserver <ChevronRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </Button>
              </div>
            </div>
          </motion.div>
