...
async function seed() {
  ...
  for (const event of events) {
    const types = await db.insert(ticketTypesTable).values([
      {
        eventId: event.id,
        name: "VIP",
  ...
  const names = ["Rakoto Jean", "Rasoa Marie", "Andry Paul", "Fanja Claire", "Hery Luc", "Miora Sophie", "Tiana René", "Lanto Pierre", "Vola Anne", "Njara Brice"];
  ...
  console.log(`Inserted payments for ${confirmedOrders.length} orders`);
